...
import {
  settingsInputStyle, SettingsField, SettingsModal, SettingsSearchBox,
  StatusPill, StatusToggle, BulkExcelButton,
} from './settings-shared';

const { useState: useStateTL } = React;

export function TwoLevelMasterPage({
  prefix,
  pageTitle,
  eyebrowAbove,
  intro,
...
function AddCategoryModal({ prefix, categoryNoun, itemNoun, categories, onClose }) {
  const [form, setForm] = useState({ short:'', name:'', desc:'', status:'Active' });
...
function AddItemModal({ prefix, itemNoun, categories, active, unitOptions, onClose }) {
  const [catShort, setCatShort] = useState(active?.short || categories[0]?.short || '');
  const [form, setForm] = useState({ name:'', spec:'', unit: unitOptions[0] || '', status:'Active' });
  const set = (k,v) => setForm({ ...form, [k]:v });
