    ...
    <div className="page">
      <div className="page-head">
        ...
        <div style={{ display:'flex', gap:8 }}>
          ...
          {canEdit && <button className="btn primary" onClick={() => setEditing('new')}>{Icons.plus} เพิ่มหน่วย</button>}
        </div>
      </div>
              ...
              return (
                <tr key={u.id}>
                  ...
                  <td style={{ textAlign:'right' }}>
                    {canEdit && <button className="btn ghost sm" style={{ padding:'2px 6px', color:'var(--ink-3)' }} onClick={() => setEditing(u)} title="แก้ไข">{Icons.edit}</button>}
                    {canEdit && <button className="btn ghost sm" style={{ padding:'2px 6px', color:'var(--clay)' }} onClick={() => remove(u)} title="ลบ">×</button>}
                  </td>
                </tr>
              );
            })}
