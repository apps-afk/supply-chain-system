...
function ScreenSettingsUnits({ go }) {
  const [q, setQ] = useStateUS('');
  const [filter, setFilter] = useStateUS('ทั้งหมด');
  const [addOpen, setAddOpen] = useStateUS(false);
...
function AddUnitModal({ onClose }) {
  const [form, setForm] = useStateUS({ symbol:'', name:'', si:'Trade', status:'Active' });
...
window.ScreenSettingsUnits = ScreenSettingsUnits;
