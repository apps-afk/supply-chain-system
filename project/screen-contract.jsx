/* global React, Icons, Chip, Stat, Av, money */
/* global SettingsSearchBox */
/*
...
function ScreenContractList({ go }) {
  const [q, setQ] = useStateCt('');
  const [filter, setFilter] = useStateCt('ทั้งหมด');
  const [uploadOpen, setUploadOpen] = useStateCt(false);
...
function UploadContractModal({ onClose, go }) {
  const [filename, setFilename] = useStateCt('');
  return (
...
function ScreenContract({ go }) {
  // phase: 'Uploaded' | 'Reviewing' | 'Reviewed' | 'Legal' | 'Final'
  const [phase, setPhase] = useStateCt('Uploaded');
  const [confirmOpen, setConfirmOpen] = useStateCt(false);
  const [legalOpen,   setLegalOpen]   = useStateCt(false);
  const [finalOpen,   setFinalOpen]   = useStateCt(false);
...
function SendToLegalModal({ onSend, onClose }) {
  const [to, setTo] = useStateCt('legal@initialestate.com');
  const [note, setNote] = useStateCt('แนบ AI Report สำหรับ CT-2024-018 — โปรดตรวจสอบประเด็นที่ AI พบและส่งความเห็นกลับมา');
  return (
    <div onClick={onClose} style={{
      position:'fixed', inset:0, background:'rgba(20,18,14,0.32)',
      display:'grid', placeItems:'center', zIndex:50,
...
window.ScreenContract = ScreenContract;
window.ScreenContractList = ScreenContractList;
