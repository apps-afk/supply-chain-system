/* global React, Icons, Chip, settingsInputStyle, SettingsField, SettingsModal, SettingsStatStrip, SettingsSearchBox, StatusPill, StatusToggle */
/*
...
function ScreenSettingsContractTypes({ go }) {
  const [q, setQ] = useStateCT('');
  const [filter, setFilter] = useStateCT('ทั้งหมด');
  const [addOpen, setAddOpen] = useStateCT(false);
...
function AddContractTypeModal({ onClose }) {
  const [form, setForm] = useStateCT({ name:'', desc:'', active:true });
...
window.ScreenSettingsContractTypes = ScreenSettingsContractTypes;
