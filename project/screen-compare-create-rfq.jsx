/* global React, Icons, Chip, Av, money, settingsInputStyle, SettingsSearchBox */
/* global SectionCard, ComparePreviewTable, AISuggestionPanel, CreateSummary, GeneratedCompare, NEXT_CMP_NO */
/* global PRICEDB_SUPPLIERS, BASE_PRICES, pricesForItem */
/*
  Compare Create — Mode B: From existing RFQs
  Flow: Pick ≥2 received RFQs (same category) → merge items → live compare with terms
*/

const { useState: useStateCMB, useMemo: useMemoCMB } = React;
...
function ScreenCompareCreateRFQ({ go }) {
  const [picked, setPicked] = useStateCMB([]); // RFQ no's
  const [generated, setGenerated] = useStateCMB(false);
...
window.ScreenCompareCreateRFQ = ScreenCompareCreateRFQ;
