/* global React, Icons, Chip, Av, money, settingsInputStyle, SettingsField, SettingsSearchBox */
/*
  Compare Create — Mode A: From Price Database
  Flow: Source (MAT/SUB) → pick Items → pick Suppliers (≥2) → live compare
*/

const { useState: useStateCMA, useMemo: useMemoCMA } = React;

const NEXT_CMP_NO = 'CMP-2025-039';

// Synthetic per-supplier price data — for demo, generated from base price
function pricesForItem(itemCode, basePrice, supplierIds) {
...
function ScreenCompareCreatePriceDB({ go }) {
  const [source, setSource] = useStateCMA('Material'); // Material | SubContract
  ...
  const [suppliersData, setSuppliersData] = useStateCMA([]);
  const [picker, setPicker] = useStateCMA(null);            // { supplierId } when item picker is open
  const [supplierPickerOpen, setSupplierPickerOpen] = useStateCMA(false);
  const [generated, setGenerated] = useStateCMA(false);

  const categories = source === 'Material'
    ? (window.MATERIAL_CATEGORIES || [])
    : (window.SUBCONTRACT_CATEGORIES || []);
  ...
  const { aiBest, fairTotals, fairMode } = useMemoCMA(() => {
...
function ItemPickerModal({ categories, source, targetSupplier, alreadyPicked = new Set(), onClose, onAdd }) {
  const [selectedCat, setSelectedCat] = useStateCMA(categories[0]?.short || '');
  const [picked, setPicked] = useStateCMA(new Set());
  const [q, setQ] = useStateCMA('');
...
function ComparePreviewTable({ items, suppliers, totals }) {
  return (
...
/* =================== AI Suggestion Panel =================== */
function AISuggestionPanel({ items, suppliers, totals, aiBest }) {
...
function CreateSummary({ rfqNo, mode, source, itemsCount, suppliersCount, canCompare, aiBest, suppliers }) {
  return (
...
function GeneratedCompare({ go, rfqMode, source, items, suppliers, totals, aiBest }) {
...
function ComparePDFPreview({ items, suppliers, totals, aiBest, winner, mode }) {
  const savingsPct = aiBest?.worstTotal ? ((aiBest.savings / aiBest.worstTotal) * 100).toFixed(1) : 0;
  ...
  return (
    <div className="card" style={{ padding:0, overflow:'hidden', boxShadow:'0 8px 32px -16px rgba(20,18,14,0.18)' }}>
      ...
      <div style={{ padding:'36px 48px', background:'#FFFFFF', minHeight:600 }}>
        ...
        {(() => {
          const roles = (window.getActiveApprovalRoles && window.getActiveApprovalRoles()) || [];
...
window.ScreenCompareCreatePriceDB = ScreenCompareCreatePriceDB;
window.PRICEDB_SUPPLIERS = PRICEDB_SUPPLIERS;
window.BASE_PRICES = BASE_PRICES;
window.pricesForItem = pricesForItem;
window.SectionCard = SectionCard;
window.ComparePreviewTable = ComparePreviewTable;
window.AISuggestionPanel = AISuggestionPanel;
window.CreateSummary = CreateSummary;
window.GeneratedCompare = GeneratedCompare;
window.NEXT_CMP_NO = NEXT_CMP_NO;
