/* global React, Icons, settingsInputStyle, SettingsField, SettingsModal, SettingsStatStrip, SettingsSearchBox, StatusPill, StatusToggle */
/*
...
window.APPROVAL_ROLES_DATA = APPROVAL_ROLES_DATA;
window.getActiveApprovalRoles = () => APPROVAL_ROLES_DATA
  .filter(r => r.status === 'Active')
  .sort((a,b) => a.order - b.order);

function ScreenSettingsApprovalRoles({ go }) {
  const [q, setQ] = useStateAR('');
  const [filter, setFilter] = useStateAR('ทั้งหมด');
  const [addOpen, setAddOpen] = useStateAR(false);
      ...
      <div className="card" style={{ marginTop:24, padding:0, overflow:'hidden' }}>
        ...
        <div style={{ padding:'36px 24px 24px', background:'#FCFAF5' }}>
          <div style={{
            ...
            {window.getActiveApprovalRoles().map(r => (
...
function AddApprovalRoleModal({ onClose }) {
  const [form, setForm] = useStateAR({ name:'', desc:'', order: APPROVAL_ROLES_DATA.length + 1, status:'Active' });
...
window.ScreenSettingsApprovalRoles = ScreenSettingsApprovalRoles;
