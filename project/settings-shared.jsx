/* global React, Icons, Chip */
/*
  Settings — shared primitives used across all master-data pages.
  Input styles, modal wrapper, stat strip, and a status pill.
*/
...
function SettingsField({ label, required, hint, children }) {
  return (
...
function SettingsModal({ title, eyebrow, onClose, onSave, children, width = 600 }) {
  return (
    <div
      onClick={onClose}
      style={{
...
function SettingsStatStrip({ stats }) {
  return (
    <div style={{
...
function SettingsSearchBox({ value, onChange, placeholder = 'ค้นหา…' }) {
  return (
    <div style={{
...
function StatusPill({ status }) {
  // status: 'Active' | 'Non-Active' | 'Closed' | 'Blacklist' | 'Planning'
  const map = {
...
/* ----- StatusToggle: segmented control for forms ----- */
...
/* ----- MultiSelectChips: pick multiple categories ----- */
function MultiSelectChips({ options, value, onChange, placeholder = 'เลือกหมวด…' }) {
...
/* ----- BulkExcelButton: opens a Bulk Excel workflow modal ----- */
function BulkExcelButton({ label = 'Bulk Excel', entity, columns, sampleRows = [] }) {
...
function BulkExcelModal({ entity, columns, sampleRows, onClose }) {
  const [step, setStep] = React.useState(1);    // 1=download, 2=upload, 3=done
...
Object.assign(window, {
  settingsInputStyle, SettingsField, SettingsModal, SettingsStatStrip, SettingsSearchBox,
  StatusPill, StatusToggle, MultiSelectChips, BulkExcelButton, BulkExcelModal,
});
