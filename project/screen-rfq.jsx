...
const { useState: useStateRFQ } = React;
...
function ScreenRFQ({ go }) {
  const [filter, setFilter] = useStateRFQ('ทั้งหมด');
  const [q, setQ] = useStateRFQ('');
...
function ScreenRFQConfirm({ go }) {
  const [items, setItems] = useStateRFQ([
...
window.ScreenRFQ = ScreenRFQ;
window.ScreenRFQConfirm = ScreenRFQConfirm;
