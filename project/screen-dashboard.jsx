/* global React, Icons, Chip, Stat, Delta, Spark, Av, money */
/*
  Dashboard — landing screen.
  Editorial. Big quiet hero number (active RFQs / price points tracked),
  followed by a price-movement section (heart of the system) and recent activity.
*/
...
function ScreenDashboard({ go }) {
  return (
    <div className="page">
      ...
      {/* Top stats row — paper-on-paper, not boxy */}
      ...
      <section style={{ marginTop: 56 }}>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 16 }}>
          <AttentionItem
            tag="สัญญาใกล้หมดอายุ"
            tone="warn"
            title="CT-2024-018 · งานติดตั้งระบบไฟฟ้า บางนา เฟส 2"
            meta="หจก. รุ่งเรืองไฟฟ้า · หมดอายุ 14 มิ.ย. 2568"
            right="28 วัน"
            onClick={() => go('contract-detail')}
          />
          <AttentionItem
            tag="ราคาผิดปกติ"
            tone="err"
            title="เหล็กเส้น DB12 สูงขึ้น +18.4% จากเดือนก่อน"
            meta="ดึงจาก RFQ-2025-019 · บริษัท เอเชียสตีล จำกัด"
            right="ตรวจสอบ"
            onClick={() => go('pricedb-detail')}
          />
          <AttentionItem
            tag="RFQ รอ Upload"
            tone="info"
            title="RFQ-2025-021 · งานก่อสร้างฐานราก ลาดพร้าว"
            meta="Supplier ส่งใบเสนอราคาแล้ว 2 วัน — รอบันทึก Price DB"
            right="Upload"
            onClick={() => go('rfq-confirm')}
          />
          <AttentionItem
            tag="ราคาไม่ได้อัพเดท"
            tone="info"
            title="กระเบื้องหลังคา CPAC โมเนียร์"
            meta="อัพเดทล่าสุด 9 ธ.ค. 2567 — เกิน 6 เดือน"
            right="ขอ Quote"
            onClick={() => go('rfq-create')}
          />
        </div>
      </section>
      ...
      <section style={{ marginTop: 64 }}>
        <div style={{ display: 'flex', alignItems: 'baseline', justifyContent: 'space-between', marginBottom: 20 }}>
          ...
          <button className="btn ghost" onClick={() => go('pricedb')}>
            ดู Price Database ทั้งหมด {Icons.chevronR}
          </button>
        </div>
        ...
        <div className="card" style={{ padding: 0 }}>
          <table className="tbl">
            ...
            <tbody>
              <PriceRow
                name="เหล็กเส้น DB12"  spec="มอก. 24-2559 · เส้น" cat="งานโครงสร้าง"
                supplier="เอเชียสตีล" supkind="aw"
                price={245} mom="+18.4%" momDir="up" yoy="+22.1%" yoyDir="up"
                spark={[140,148,151,150,165,172,180,195,210,228,240,245]} flag="err"
                onClick={() => go('pricedb-detail')}
              />
              <PriceRow
                name="ปูนซีเมนต์ ตราช้าง"  spec="ปอร์ตแลนด์ · ถุง 50 กก." cat="งานโครงสร้าง"
                supplier="SCG Distribution" supkind="sc"
                price={165} mom="+1.9%" momDir="up" yoy="+3.2%" yoyDir="up"
                spark={[155,158,159,160,160,161,162,162,163,164,164,165]}
              />
              <PriceRow
                name="กระเบื้องเซรามิค 60×60"  spec="Grade A · ตร.ม." cat="งานพื้น-ผนัง"
                supplier="ไทยเซรามิค" supkind="default"
                price={285} mom="−4.2%" momDir="down" yoy="−1.1%" yoyDir="down"
                spark={[298,302,300,298,295,294,292,290,290,288,287,285]}
              />
              <PriceRow
                name="อิฐมวลเบา Q-CON"  spec="7.5×20×60 cm · ก้อน" cat="งานก่ออิฐ-ฉาบปูน"
                supplier="Q-CON Direct" supkind="default"
                price={28} mom="0.0%" momDir="flat" yoy="+5.2%" yoyDir="up"
                spark={[26.6,26.8,27,27.2,27.5,27.8,28,28,28,28,28,28]}
              />
              <PriceRow
                name="สีน้ำพลาสติก TOA SuperShield"  spec="ภายนอก · แกลลอน" cat="งานสี"
                supplier="TOA Distribution" supkind="default"
                price={1180} mom="+2.6%" momDir="up" yoy="+8.7%" yoyDir="up"
                spark={[1085,1100,1110,1115,1130,1140,1145,1150,1155,1165,1170,1180]}
              />
            </tbody>
          </table>
        </div>
          ...
          <table className="tbl">
            <tbody>
              {[
                ...
                <tr key={i}>
                  <td style={{ width: '24%' }}>
                    <div className="font-mono" style={{ fontSize: 12, color: 'var(--ink-2)' }}>{r.no}</div>
                  </td>
                  <td>{r.title}</td>
                  <td>
  ...
  return (
    <tr onClick={onClick} style={{ cursor: 'pointer' }} className={flag ? `row-flag-${flag}` : ''}>
      <td>
        ...
        <div style={{ fontSize: 11.5, color: 'var(--ink-3)', marginTop: 2 }}>{spec}</div>
      </td>
      <td style={{ fontSize: 12.5, color: 'var(--ink-2)' }}>{cat}</td>
      <td>
        <span style={{ display: 'inline-flex', gap: 8, alignItems: 'center' }}>
          <Av initials={supplier.slice(0, 2)} kind={supkind} />
          <span style={{ fontSize: 12.5 }}>{supplier}</span>
        </span>
      </td>
